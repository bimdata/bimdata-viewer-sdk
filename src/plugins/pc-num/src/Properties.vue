...
export default {
  ...
  methods: {
    ...
    async fetchProperties(classification) {
      ...
      const ifcPropsRequests = (response.classificationProperties || [])
        .filter(prop => !(prop.name && prop.name.includes('Uniformat'))) // skip classifications for now
        .map(prop => {
          const options = {
            params: {
              namespaceUri: prop.propertyNamespaceUri,
            },
          };
          return requestApi("/Property/v2", "GET", options);
        });
      const rows = (await Promise.all(ifcPropsRequests)).map(prop => {
        prop.value = undefined;
        prop.currentValue = undefined;
        prop.toUpdate = false;
        if (prop.name === "Material") {
          prop.propertySet = "material";
        } else if (prop.name === "Name") {
          prop.propertySet = "attributes";
        } else {
          prop.propertySet = "BaseQuantities";
        }
        return prop;
      });
    ...
    updateRowsWithCurrentProperties() {
      ...
      this.rows.forEach(row => {
        if (row.propertySet === "attributes") {
          const currentProp = this.displayedElementDetails.attributes.properties.find(
            prop => prop.definition.name === row.name
          );
          if (currentProp) {
            row.currentValue = currentProp.value;
          }
        } else if (row.propertySet === "material") {
          const list = this.displayedElementDetails.material_list;
          if (list && list.length) {
            row.currentValue = list.map(elem => elem.material.name).toString();
          } else {
            row.currentValue = 'Vide';
          }
        } else {
